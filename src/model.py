...
def get_prefix_weights(start: int, end: int, total: int, schedule: PrefixAttentionSchedule, exp_alpha: float = 1.0) -> jax.Array:
    ...
    `exp_alpha` controls the steepness of the exponential decay (only used when schedule == "exp").
    Higher values produce steeper decay. Default 1.0 matches the original paper.
    """
    ...
    if schedule == "ones":
        ...
        if schedule == "exp":
            w = w * jnp.expm1(exp_alpha * w) / jnp.expm1(exp_alpha)
    else:
        ...
        def step(carry, _):
            ...
            def pinv_corrected_velocity(obs, x_t, y, t):
                ...
                weights = get_prefix_weights(
                    inference_delay, prefix_attention_horizon, self.action_chunk_size, prefix_attention_schedule, exp_alpha
                )
